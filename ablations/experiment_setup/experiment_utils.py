...
def loss_and_metrics(logits, targets, loss_type, aux_loss, aux_weight, aux_logits):
    ...
    if loss_type == 'cross_entropy':
        ...
        flat_logits = logits.reshape(-1, V) # Flatten to (B*T, V)
        flat_targets = targets.reshape(-1) # Flatten to (B*T,)

        main_loss = optax.softmax_cross_entropy_with_integer_labels(flat_logits, flat_targets) # Obtain (B*T,) array of losses
        ...
        main_loss = optax.softmax_cross_entropy_with_integer_labels(last_logits, last_targets) # scalar loss at last position
...
def train_step(model, params, opt_state, x, y, tx, *, rng, loss_type, aux_loss, aux_weight):
    ...
    def loss_fn(params):
        ...
        loss, metrics = loss_and_metrics(final_logits, y, loss_type, aux_loss, aux_weight, aux_logits)
